#!/bin/bash
# kilde M0 統合テスト (ローカル実行用)
#
# 使い方:
#   scripts/integration-test.sh
#
# 前提:
#   - 画面収録・マイクの権限が付与済みであること (未許可なら先に `spike doctor`)
#   - 音量が 0 (ミュート) だと音声シナリオが失敗します
#
# 内容: spike の全コマンドを実際に録画しながら動かし、
#       出力ファイルのトラック構成と RMS を機械検証する。

set -u

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SPIKE="$ROOT/.build/debug/spike"
...
cleanup() {
    if [ "$MONITOR_SET_UP" = "1" ]; then
        "$SPIKE" monitor teardown >/dev/null 2>&1 && echo "[cleanup] 既定出力を復元しました"
    fi
...
inspect() { "$SPIKE" inspect "$1" 2>/dev/null; }

field() { # field <file> <正規表現> <番号> — マッチ行から "key=value" を抽出
    inspect "$1" | grep "$2" | head -"$3" | grep -o '[0-9.]*' | head -1
}
...
record_with_voice() { # record_with_voice <spikeの引数...> — 録画を bg で開始し音声を鳴らす
    "$SPIKE" "$@" --duration "$DUR" &
    REC_PID=$!
    sleep "$DELAY"
...
[ -x "$SPIKE" ] || { echo "spike バイナリなし"; exit 1; }
...
if "$SPIKE" doctor > "$WORK/doctor.txt" 2>&1 \
    && grep -q "preflight: true" "$WORK/doctor.txt" \
...
log "T2: devices (機器列挙)"
"$SPIKE" devices > "$WORK/devices.txt" 2>&1
if [ $? -eq 0 ] && grep -q "== displays ==" "$WORK/devices.txt" \
...
# ---- T3: rec-system (画面 + システム音声) -----------------------------------

log "T3: rec-system — 画面 + システム音声 (${DUR}s)"
F="$WORK/t3-rec-system.mov"
if record_with_voice rec-system --output "$F"; then
    if grep -q "video: present" <(inspect "$F") && rms_above "$F" 0.005; then
        ok "T3 rec-system: 映像あり・音声あり (rms=$(rms_of "$F"))"
    else
        bad "T3 rec-system: ファイル内容が不正 (rms=$(rms_of "$F"))"
    fi
else
    bad "T3 rec-system: コマンド失敗"
fi

# ---- T4: rec-system --mic (2 音声トラック) ----------------------------------

log "T4: rec-system --mic — システム + マイクの 2 トラック (${DUR}s)"
F="$WORK/t4-rec-system-mic.mov"
if record_with_voice rec-system --mic --output "$F"; then
    N=$(inspect "$F" | grep -c '^audio\[')
    if [ "$N" -ge 2 ] && grep -q "video: present" <(inspect "$F"); then
        ok "T4 rec-system --mic: 2 音声トラック共存"
    else
        bad "T4 rec-system --mic: トラック数=$N (2 以上が必要)"
    fi
else
    bad "T4 rec-system --mic: コマンド失敗"
fi

# ---- T5: rec-audio-only (音声のみ) -------------------------------------------

log "T5: rec-audio-only — SCK 音声のみ (${DUR}s)"
F="$WORK/t5-audio-only.m4a"
if record_with_voice rec-audio-only --output "$F"; then
    if grep -q "video: absent" <(inspect "$F") && rms_above "$F" 0.005; then
        ok "T5 rec-audio-only: 映像なし・音声あり (rms=$(rms_of "$F"))"
    else
        bad "T5 rec-audio-only: 内容不正 (rms=$(rms_of "$F"))"
    fi
else
    bad "T5 rec-audio-only: コマンド失敗"
fi
...
start_soundapp
if "$SPIKE" rec-window --match SpikeSoundWindow --duration "$DUR" --output "$F" >/dev/null 2>&1; then
    ...
    bad "T6 window 包含: コマンド失敗"
fi

log "T7: rec-window — 他アプリの音は入らない (${DUR}s)"
# 音を出しているのは soundapp。無関係なウィンドウ (Wallpaper / ゴミ箱) を収録する
UNRELATED=""
if "$SPIKE" devices 2>/dev/null | grep -q "Wallpaper"; then
    UNRELATED="Wallpaper"
elif "$SPIKE" devices 2>/dev/null | grep -q "ゴミ箱"; then
    UNRELATED="ゴミ箱"
fi
F="$WORK/t7-window-negative.mov"
if [ -n "$UNRELATED" ]; then
    if "$SPIKE" rec-window --match "$UNRELATED" --duration "$DUR" --output "$F" >/dev/null 2>&1; then
        RMS=$(rms_of "$F")
...
log "T8: rec-audio-only --match — 特定アプリの音声のみ (${DUR}s)"
F="$WORK/t8-audio-window.m4a"
start_soundapp
if "$SPIKE" rec-audio-only --match SpikeSoundWindow --duration "$DUR" --output "$F" >/dev/null 2>&1; then
    if grep -q "video: absent" <(inspect "$F") && rms_above "$F" 0.005; then
    ...
    bad "T8 audio+window: コマンド失敗"
fi
stop_soundapp

# ---- T9: BlackHole ループバック ----------------------------------------------

log "T9: monitor + rec-device — マルチ出力経由の BlackHole 録音 (${DUR}s)"
if "$SPIKE" devices 2>/dev/null | grep -q "BlackHole"; then
    if "$SPIKE" monitor setup >/dev/null 2>&1; then
        MONITOR_SET_UP=1
        F="$WORK/t9-blackhole.m4a"
        "$SPIKE" rec-device --device BlackHole2ch_UID --duration "$DUR" --output "$F" >/dev/null 2>&1 &
        REC_PID=$!
        sleep "$DELAY"
        afplay "$VOICE" >/dev/null 2>&1
        wait "$REC_PID"
        ...
        if "$SPIKE" monitor teardown >/dev/null 2>&1; then
            MONITOR_SET_UP=0
            if "$SPIKE" monitor status 2>/dev/null | grep "default output:" | grep -qv "kilde"; then
...
"$SPIKE" rec-system --duration 60 --output "$F" >/dev/null 2>&1 &
REC_PID=$!
sleep 4
kill -INT "$REC_PID"
wait "$REC_PID"
...
if [ "$EXIT_CODE" = "0" ] && num_between "${VD:-0}" 2.5 8; then
    ...
    bad "T10 SIGINT: exit=$EXIT_CODE duration=${VD:-N/A}s"
fi
