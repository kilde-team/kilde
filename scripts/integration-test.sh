...
if [ "$T17_ENV_OK" = "1" ]; then
    ...
    if "$KILDE" rec --exclude-app "$EXCL_ID" --duration "$DUR" --output "$F" > "$WORK/t17.log" 2>&1; then
        ...
        if awk -v v="${RMS:-1}" 'BEGIN{exit !(v < 0.00005)}'; then
            ...
            # 「無音でない」を返し、退行判定に届かなくなる。録音ウィンドウの中で
            # 完結する短い外来音は前後のプローブのどちらにも載らないが、#83 の干渉源
            # (soundapp の連続ループ / record_with_voice の ~4 秒 afplay) はいずれも
            # 録画 6 秒をまたぐか再プローブまで残るため、この窓は既知の限界として残す
            stop_excl_app
