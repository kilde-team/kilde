...
> **実装 (issue #197) — 会議の自動録画:** オンライン会議 (Zoom / Google Meet /
> Microsoft Teams / Slack ハドル / Webex) の開始を検知し、**その会議ウィンドウだけ**を
> システム音声 + マイク (ミックスは選択中のトラック方針) で録り、会議が終わったら止める。
> 既定はオフで、パネルの「会議の自動録画」で有効にする (設定は UserDefaults。CLI と共有の
> `config.json` はスキーマがエンジン側にあるため使わない)。
>
> - **検知 = 2 つの信号の AND** (`MeetingDetector.evaluate`、純関数):
>   (1) 会議アプリのプロセスが**マイク入力を使っている** — CoreAudio のプロセスオブジェクト
>   (`kAudioHardwarePropertyProcessObjectList` + `kAudioProcessPropertyIsRunningInput`、
>   **macOS 14.2+**。未満では機能を無効化して理由を出す)。ヘルパープロセス
>   (`com.google.Chrome.helper` 等) が音声を持つので bundleID は前方一致で見る。
>   (2) そのアプリに**会議ウィンドウがある** — `CGWindowListCopyWindowInfo` のタイトルと所有者。
>   Zoom は会議ウィンドウ名 (言語別)、ブラウザは Meet のタブタイトル (`Meet - …`) が必須。
>   Teams / Webex はタイトルが会議名で決まらないため «会議らしい語 → 最新のウィンドウ»、
>   Slack はハドルがメインウィンドウ内なので «最前面のウィンドウ» に倒す。
>   WebKit の GPU / WebContent プロセス (Safari・新 Teams が共有) は帰属があいまいなので、
>   **タイトル一致が無ければ会議とみなさない** (Safari でマイクを使っただけで常駐中の
>   Teams を録り始めないため)
> - **ウィンドウの列挙に SCK を使わない。** 検知は 2 秒周期で常時回るが、
>   `SCShareableContent` の列挙は録画開始と重なると両方が無期限に止まる (issue #70)。
>   CGWindowList は replayd を経由せず、CGWindowID は `SCWindow.windowID` と同じ値なので
>   そのまま `RecordRequest.Target.window(id:)` に渡せる。録画の開始自体は手動と同じ
>   `RecordingController.start` で、パネルの列挙 (`enumerationsRunning`) が走っている間は
>   開始を次の周期に送る
> - **誤検知の抑止**: 2 周期連続 (約 2〜4 秒) で同じウィンドウを検知してから始める。
>   試みた会議ウィンドウは**閉じるまで再試行しない** (失敗の連続通知を防ぎ、ユーザーが
>   自動録画を手で止めたら同じ会議では録り直さない)。手動録画中に始まった会議も同様
> - **終了判定**: 会議ウィンドウが 2 周期続けて存在しない (画面外・他スペースも «存在» に
>   含める — Zoom は画面共有中に会議ウィンドウを隠すため)、または会議アプリの音声が
>   入力・出力とも 60 秒止まった。停止は手動と同じ `Recorder.stop()` なので
>   **ファイナライズの経路は 1 本のまま**。自動で始めた録画だけを自動で止め、手動の録画は止めない
> - マイクが未許可なら許可ダイアログは出さず (会議の操作を邪魔しない)、システム音声だけで録って
>   通知で伝える。自動で録画が始まったことは**必ず通知する** (本人の操作なしに他人の声を録るため)
> - 判定規則は `KILDE_GUI_SELFTEST_MEETING=1` が合成データで縛る (docs/DEVELOPMENT.md §3)
