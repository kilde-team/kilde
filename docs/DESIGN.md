...
- ロックは**ユーザー単位の一時ディレクトリ**に置く (`NSTemporaryDirectory()`)。
...
- 保持者が異常終了した場合に備え、PID (`kill(pid, 0)`) と経過時間 (30 秒) で孤児を検出して
  片付ける。PID の再利用で「生存」に見える場合があるため両方で判定する。
  解放時は inode を照合し、**自分が作ったロックだけを消す**
