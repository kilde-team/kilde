...
**5/5 は 1.8% では説明できない** (5 組中 2 件以上が出る確率は約 0.31%)。`git stash` で
自分の変更を外した main 相当でも同じ 4/5 になり、**コードではなく環境**だと切り分けてある。
このとき `replayd` は 11 時間以上、SCK を使う常駐 GUI は 1 時間半稼働していた。

したがって **T24 が FAIL しても、まず #90 の回帰を疑ってはいけない。**
`replayd` の稼働時間と常駐 SCK クライアントの有無を先に確認すること
(`ps -Ao pid,etime,comm | grep -E 'replayd|KildeGUI'`)。スクリプトの FAIL 文と
この節の両方が「#90 の回帰」へ誘導する書き方になっているが、**環境要因の確認が先**である。
...
**「10 回試して 0 件」を解決の根拠にしないこと。** 残存率 1.8% に対して n=10 で
全通する確率は **83%**、n=40 でも 48% ある。#90 の受け入れ条件はこの「10 回で 0」
だったため**残存を 8 割方見逃す設計**になっており、実際に交互計測 0/10 を根拠に
「解消した」と PR に書いて撤回する事故が起きた (PR #98)。#99 は **100 回**にしている。
