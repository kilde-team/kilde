...
**これで消え切ってはいない。約 1.8% が残る (issue #99)。**

| バイナリ | 併走時の `rec` 失敗 |
|---|---|
| 列挙ロック導入前 (`5b22465`) | **7/10** (交互計測) / **4/5** (T24 と同じ形) |
| 導入後 | **2/110 = 1.8%** (95% 信頼区間 [0.22%, 6.41%]。Clopper-Pearson) |

**残存の機序は特定できていない。** 列挙ロックの排他自体は効いており (時刻付き `lsof` で
同一時刻に 2 プロセスが保持している行は 0 件)、スリープ・`-3818` との混同・
直前の SCK 酷使など **6 つの仮説はいずれも実測で否定済み** (詳細は #99)。
再現性が極めて低く、クリーンな環境では 40 回連続で当てても 0 件だった。

統合テストは**残存を許容しつつ回帰は捕まえる**形にしてある — T24 は 5 組回し、
**2 組以上で `-3801` なら FAIL、1 組だけなら SKIP** (`scripts/integration-test.sh`)。

**ただし発生率は一定ではない。環境で桁が変わる。** 上の 1.8% を独立試行の確率として
扱うと読み誤る:
...
**5/5 は 1.8% では説明できない** — 独立試行なら `0.018⁵ ≈ 1.9×10⁻⁹`、**約 5 億回に 1 回**
である。当時の作業ツリーと、`git stash` でそこから未コミット分を外した状態を
同一環境で連続して測ったところ、**どちらも同じ 4/5** だった。このとき `replayd` は
11 時間以上、SCK を使う常駐 GUI は 1 時間半稼働していた。

**この比較の射程には限りがある** (DEVELOPMENT.md の手順 3 と同じ理由)。`git stash`
が外せるのは未コミット分だけで、**ブランチにコミット済みだった #95 の変更
(`551ba29f` の停止シーケンス並べ替え) は両方の測定に残っていた** — 厳密には
「main 相当」ではなく「`551ba29f` 相当」との比較である。それでも結論が変わらないのは、
**#95 の変更が停止経路にしか触れていない**ためで、`startCapture` /
`SCKStartupLock.acquire` / `usesStartupLock` は素のままだった。T24 が見るのは**起動側**
なので、残っていた変更は `-3801` の発生率に効かない。つまりこの 4/5 は
「無変更との比較」ではなく「**起動側に無関係な変更との比較**」として読むこと。

したがって **T24 が FAIL しても、まず #90 の回帰を疑ってはいけない。**
`replayd` の稼働時間と常駐 SCK クライアントの有無を先に確認すること
(`ps -Ao pid,etime,comm | grep -E 'replayd|KildeGUI'`)。`bad` 文と DEVELOPMENT.md の
T24 行も、かつては「#90 の回帰」と断定していたが、**環境要因の確認を先に促す文面に
改めた** (この節と同じ PR)。

**ただし「環境のせい」で終わらせないこと。** 稼働時間が長いと分かっただけでは
回帰がないことの証明にはならない — **環境を整えて (常駐 SCK クライアントを終了し、
`replayd` をリセットして) 測り直し、その結果で判定する**。整えた環境でも FAIL が
続くなら回帰を疑う。**元の FAIL は、測り直しの結果が出るまで未解決のまま扱う。**
判定に使った環境条件と各組の結果は記録に残すこと (手順は DEVELOPMENT.md の T24)。

回帰の検出力も「捕まる / 捕まらない」の二択ではない。導入前の実測 (4/5、p≈0.8) なら
約 99.3% で捕まるが、**中程度の回帰は素通りする** — p=0.2 なら FAIL は 26% にとどまり、
41% は SKIP、33% は PASS になる。**1 回の PASS は「回帰なし」の証明にならない。**

**「10 回試して 0 件」を解決の根拠にしないこと。** `p=0.018` を固定して独立試行と
みなした**参考値**で言えば、n=10 で全通する確率は **83%**、n=40 でも 48% ある
(上で述べたとおり実際の `p` は環境で動くので、これは「10 回では足りない」ことを
示すための概算であって、必要試行数の保証ではない)。#90 の受け入れ条件はこの
「10 回で 0」だったため**残存を 8 割方見逃す設計**になっており、実際に交互計測 0/10 を
根拠に「解消した」と PR に書いて撤回する事故が起きた (PR #98)。#99 は **100 回**にしている。
**想定される失敗率から必要な試行数を逆算すること。**

**そして試行数だけでも足りない。** 上の環境依存があるため、**条件を固定しない 100 回は
「たまたま良い時間帯だった」と区別できない**。#99 を測るときは `replayd` の稼働時間や
常駐 SCK クライアントの有無を併せて記録すること。
