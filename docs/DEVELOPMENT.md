...
| T17 | `rec --exclude-app` — 除外したアプリの音が出力に入らない (**判定前に 1 秒の無音プローブで環境を確認、鳴っていたら判定不能として SKIP** — ディスプレイ全体を録るため他の音源が混入する。issue #83) / `--window` 複数指定でディスプレイ全体の大きさで録れ、音声スコープも効く (含めたアプリの音は入り、含めなかったアプリの音は入らない) / 実行中でない bundleID は exit 3 / 併用不可の組合せは録画前に exit 64 |
