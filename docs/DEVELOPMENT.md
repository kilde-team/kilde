...
| T17 | `rec --exclude-app` — 除外したアプリの音が出力に入らない (**無音プローブ (事前 + 混入検出時の事後) で環境を確認、鳴っていたら/鳴り始めていたら判定不能として SKIP** — ディスプレイ全体を録るため他の音源が混入する (SKIP 経路: 事前プローブが無音でない / プローブ録音失敗・rms 不明 / 録音への混入検出後に環境が汚れていた)。issue #83) / `--window` 複数指定でディスプレイ全体の大きさで録れ、音声スコープも効く (含めたアプリの音は入り、含めなかったアプリの音は入らない) / 実行中でない bundleID は exit 3 / 併用不可の組合せは録画前に exit 64 |
