...
| T17 | `rec --exclude-app` — 除外したアプリの音が出力に入らない (**無音プローブ (事前 + 混入検出時の事後) で環境を確認、鳴っていたら/鳴り始めていたら判定不能として SKIP**。「静か」のしきい値は 0.005 — この機の SCK システム音声には rms≈0.0012 のフロアが乗ることがあり (出力切替では消えない)、実音声の混入 (実測 0.06〜0.13) とは 0.005 で分離する — ディスプレイ全体を録るため他の音源が混入する (SKIP 経路: 事前プローブが無音でない / プローブ録音失敗・rms 不明 / 本録音の結果を検査できない (rms 不明) / 混入検出後の再プローブ失敗 / 混入検出後に環境が汚れていた / 事後プローブの rms 不明。録音中に始まり終わる短い外来音は検出できず、その場合は退行として FAIL になるが、メッセージに環境起因の可能性を注記している)。issue #83) / `--window` 複数指定でディスプレイ全体の大きさで録れ、音声スコープも効く (含めたアプリの音は入り、含めなかったアプリの音は入らない) / 実行中でない bundleID は exit 3 / 併用不可の組合せは録画前に exit 64 |
...
| T24 | `devices` 併走 (issue #90) — `kilde devices` の列挙と `rec` の SCK 起動が重なっても `-3801` で失敗しない。5 組回し、**2 組以上で `-3801` なら FAIL** (回帰)、**1 組だけなら SKIP** (issue #99 の残存。実測 1.8% で、5 組中 2 件以上は約 0.31% でしか起きない)。修正前のバイナリ (`5b22465`) では 4/5 が `-3801` になることを実測し、門になることを確認済み |
