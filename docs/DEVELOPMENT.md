...
| T22 | `rec --audio mic` — 起動直後の SIGINT でも **exit 0** (issue #56)。**ファイルの有無はどちらでも正しい** — 準備フェーズで中断できれば出力なし、録画開始後に安全停止へ解けたなら再生可能なファイルが残る (後者はファイルが空/不完全でないことまで確認する)。どちらに解けたかはログに記録される |
