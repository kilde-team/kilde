...
| T13 | `rec --hotkey` — 待機中の SIGINT は録画を始めず exit 0・出力ファイルなし (ホットキーの押下自体は目視確認)。**exit 0 だけでは通らない** — SIGINT 単独で 5 秒以内に終了することまで見る (issue #101)。SIGTERM は SIGINT と同じ安全停止ハンドラに繋がるため、TERM への昇格で exit 0 になった回は「INT が効いた」ことの証拠にならない |
...
| T18b | 同秒の 2 本同時起動で互いのファイルを消さない (共存を検証)。**判定は 3 軸に分かれる** — 名前が 2 つ残らなければ FAIL (予約の保護 = issue #59)、**出力が再生可能でなければ FAIL** (`inspect` で `rms` が読め、`duration` が 1 秒以上、`values` が 1 以上。停止シーケンスの順序を誤ると未完了ファイルが残る = issue #95)。**`rms` の値に閾値は置かない** — `%.4f` は 0.00005 までを `0.0000` に丸めるため、無音環境の正常な録音を落としてしまう。「サンプルが来たか」は丸めの影響を受けない `values` で見る (issue #108)、**プロセスが自力で終わらないだけなら SKIP** (`SCStream.stopCapture()` が replayd から戻らない = issue #103。kilde 側では解消できないため、これで無関係な PR を赤くしない)。#103 が閉じたら SKIP の分岐を FAIL に上げる |
...
| T22 | `rec --audio mic` — 起動直後の SIGINT でも **exit 0** (issue #56)。**ファイルの有無はどちらでも正しい** — 準備フェーズで中断できれば出力なし、録画開始後に安全停止へ解けたなら再生可能なファイルが残る (後者はファイルが空/不完全でないことまで確認する)。どちらに解けたかはログに記録される。**exit 0 だけでは通らない** — T13 と同じく SIGINT 単独で 10 秒以内に終了することまで見る (issue #101)。これが満たされない場合、#67 の回帰かファイナライズの異常な遅延かはログで切り分ける |
| T24 | `devices` 併走 (issue #90) — `kilde devices` の列挙と `rec` の SCK 起動が重なっても `-3801` で失敗しない。5 組回し、**2 組以上で `-3801` なら FAIL**、**1 組だけなら SKIP** (issue #99 の残存)。修正前のバイナリ (`5b22465`) では 4/5 が `-3801` になることを実測し、門になることを確認済み。**FAIL したら下の切り分け手順に従うこと — 回帰と決めつけない** |
| T25 | 設定 `hotkey` + `--duration` (issue #97) — 設定ファイル由来のホットキーがあると `--duration` を明示しても待機モードに入る。`< /dev/null` の非対話実行でも待機が終わらないこと、stderr に WARNING が出ること、出力ファイルを作らないこと、待機中の SIGINT で **exit 0** になることを見る。猶予は T23 と同じ 15 秒 (起動に `SCKStartupLock` の待機が最大 15 秒乗りうるため)。キーは T13 (f11) / T21 (f9) / T23 (f10) と分けてある (T25 は f8) |
...
### T24 が FAIL したときの切り分け

**`-3801` の発生率は環境で桁が変わる** (同一バイナリで 0/5 → 1/5 → 5/5 を実測。
DESIGN.md「列挙も同じロックで直列化する」参照)。**FAIL をそのまま #90 の回帰と
読まないこと。** 順に確認する:

1. **環境を記録する** — `ps -Ao pid,etime,comm | grep -E 'replayd|KildeGUI'` で
   `replayd` と常駐 SCK クライアントの稼働時間を控える。11 時間規模で悪化した実測がある
2. **各組の結果を控える** — `bad` 行の `[組N rec exit=…]` と
   `$WORK/t24-rec-*.log` / `$WORK/t24-devices-*.log`。`-3801` と他の失敗を分けて数える
   (作業ディレクトリは自動削除されないので、判定を終えるまで消さない)
3. **自分の変更が原因か切り分ける** — **main 相当のバイナリで同じ条件を測り、
   同じ率なら環境**である。**`git stash` が使えるのは未コミットの変更だけ**で、
...
5. **判定** — 整えた環境で FAIL が続くなら**回帰を疑い、調査する**。0〜1 組に戻るなら
   環境要因として記録する (#99 に追記すると次の人の役に立つ)

**手順 4 を飛ばして「環境のせい」で閉じないこと。** 稼働時間が長いと分かっただけでは
回帰がないことの証明にはならない。**元の FAIL は、測り直しの結果が出るまで
未解決のまま扱う。**
