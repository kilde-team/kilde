...
| S7 | 環境差異 | ⚠️ | macOS 26 のみ実挙動を検証。14/15 向け: CI (macos-15 ランナー) で `swift build` (debug/release) が通ることを継続検証 (issue #4 の自動化側)。実挙動 (統合テスト T5〜T8) は引き続き実機検証が残る |
...
### F-I: 会議アプリ実地検証 S10 — Google Meet で実測 (2026-09-12)

`kilde rec --preset meeting` を実際の Google Meet (Chrome) の会議で実行:

- **ウィンドウ選択**: プロンプトの一覧から `com.google.Chrome "Google Meet"` を選択。
  録画された映像は 1798x1342 で選択ウィンドウと一致 (他アプリ・他タブは混入なし)
- **音声**: 30.5 秒の録画に音声トラック 1 本 (mixed)。RMS 0.0349 —
  再生して**自分の声と相手の声の両方が入っていることを人間が確認**
- **マイク共有**: 録画中も会話が普通に継続 (取り合いによる無音化なし)。
  マイクは kilde と Meet が同時に開いても競合しない (AVCaptureSession は
  複数プロセスからの読み取りを許容する)
- **ファイル**: 4.9 MB / 30.49s、`kilde inspect` で video present・audio[0] の
  トラック構成は統合テスト (T3/T4b) と同じ形

**未検証**: Zoom / Microsoft Teams。収録経路 (SCK のウィンドウスコープ + マイクの
AVCaptureSession) はアプリ非依存のため、問題が起きるとすれば各アプリのマイク排他
(独占的なオーディオセッションを持つ実装) の場合に絞られる。既知の会議アプリで
macOS でマイク排他をするものは少ないが、Zoom の「オーディオ設定で独占モードを
有効化」オプションには注意。
...
7. 残課題: (a) 旧 OS (13/14/15) での S8/S9 挙動、(b) 長時間ドリフト。
   ~~(c) S10 会議アプリ実地検証~~ → **Google Meet で実測済み (上記 F-I)**。
   Zoom / Teams は未検証だが、経路は Chrome (SCK ウィンドウスコープ + マイク) と
   同じため、問題が起きるとすれば各アプリのマイク排他の実装差に絞られる。
