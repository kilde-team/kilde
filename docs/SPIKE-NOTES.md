...
- `kilde rec --window KildeDriftMarker --audio system --audio mic --audio-tracks <mode>` で収録する。
  ウィンドウ収録なので system 音声はこのアプリのビープだけになり (F-B)、マイクはスピーカーから
  回り込んだ同じビープを拾う。映像・system・mic の 3 系統に同じ瞬間のマーカーが入る
...
- 前提: スピーカー音量 > 0 (イヤホン・ヘッドホン不可)、静かな環境、計測中はウィンドウを隠さない・スリープさせない
- **マイクが実際に音を拾えること**。MacBook を閉じて外部ディスプレイで使う (クラムシェル) と
  内蔵マイクはデジタル無音 (peak 0.000) になる — 2026-09-11 の手順確認で実際に発生した。
  その場合は `MIC="device:<名前>" scripts/drift-test.sh` で Web カメラ等のマイクを指定する
...
|------|------|------------|-------|---------------------|------------------|--------------------|------|
| — | — | 15m / 30s | separate | 未計測 | 未計測 | 未計測 | 依頼者の実機で実施予定 |
| — | — | 15m / 30s | mixed | 未計測 (mixed 1 本) | — | — | 同上 |
...
mic 側は**スピーカーの音を直接拾えるマイク** (蓋を開けた MacBook の内蔵マイク + 内蔵スピーカー等) で行うこと。
